...
drought_exp_clean <- drought_exp |> 
  # Clean tha names (underscores and lowercases)
  clean_names() |> 
  # Changes the names of the rows in the column specie
  ...
  # Changes the names of the elements of the column water
  mutate(water_treatment = case_match(
    water,
    "WW" ~ "Well watered",
    "DS" ~ "Drought stressed"
  ))  |> 
  # as_factor makes a variable categorical so it can be used in models 
  # And relevel gives order
  ...
  mutate(water_treatment = as_factor(water_treatment),
         water_treatment = fct_relevel(water_treatment,
                                       "Drought stressed",
                                       "Well watered")) |> 
  #Select only the columns in the parentesis 
...
ggplot(data = drought_exp_clean, aes(x = sla, y = total_g)) + 
  geom_point() + 
labs(x = "Specific Leaf Area cm\U00B2/(g)", y = "Total mass (g)")
```
...
```{r}
ggplot(data = drought_exp_clean, aes(x = water_treatment, y = total_g, color = water_treatment)) +
  geom_jitter(hight = 0, width = 0.3) +
  labs(x = "Water Treatment", y = "Total mass (g)", title = "Well watered plants tend to have a higher mas") 
```
...
```{r}
ggplot(data = drought_exp_clean, aes(x = reorder(species_name, -total_g), y = total_g, color = species_name )) +
  geom_jitter(hight = 0, width = 0.3) +
  labs(x = "Water Treatment", y = "Total mass (g)", title = "Well watered plants tend to have a higher mas") +
